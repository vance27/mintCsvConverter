...
// a separate "<code> /<itemCode> <amount>-" line referencing the item
// above it (e.g. "380381  /1774692   14.00-" under "1774692 WEED&FEED
// 74.99") — a real ingest showed the model misreading this as its own
// line item instead of folding it into the referenced item's
// discountAmount, which reconcile.ts can't catch after the fact (the
// arithmetic can coincidentally still balance), so the prompt below now
// spells out the exact "/"-prefix and trailing "-" convention with a
// worked example rather than describing discounts only in general terms.
// The footer gives
...
- A discount/instant-savings line prints as its own line directly below the item it discounts, in the form "<code2> /<itemCode>  <amount>-" — e.g. "380381  /1774692   14.00-" below "1774692 WEED&FEED 74.99 N". The "/" prefixes a reference back to the item code printed above it, and the trailing "-" marks it as a subtraction from that item's price, not an addition. This referencing line is NEVER its own line item — do not add it to "items". Instead, attribute its amount (as a positive number) to the referenced item's "discountAmount", and leave that item's own "lineTotal" as its original printed extended price (do not subtract the discount from lineTotal yourself).
...
Example: the two printed lines "1774692 WEED&FEED 74.99 N" then "380381  /1774692   14.00-" together produce ONE item: itemCode "1774692", rawName "WEED&FEED", lineTotal 74.99, discountAmount 14.00, taxable false — NOT two items, and NOT an item with lineTotal 60.99.
