    ...
    await prisma.$transaction(async (tx) => {
        ...
        if (snapshot.priceObservations.length > 0) {
            await tx.priceObservation.createMany({
                data: snapshot.priceObservations.map((o) => ({ ...o, observedAt: new Date(o.observedAt) })),
            });
        }
        const payerExclusionRules = snapshot.payerExclusionRules ?? [];
        if (payerExclusionRules.length > 0) {
            await tx.payerExclusionRule.createMany({
                data: payerExclusionRules.map((r) => ({ ...r, createdAt: new Date(r.createdAt) })),
            });
        }
        const variableSplitRules = snapshot.variableSplitRules ?? [];
        if (variableSplitRules.length > 0) {
            await tx.variableSplitRule.createMany({
                data: variableSplitRules.map((r) => ({ ...r, createdAt: new Date(r.createdAt) })),
            });
        }
