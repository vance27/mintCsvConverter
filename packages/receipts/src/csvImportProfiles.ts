...
const columnRefSchema = z.union([
    z.object({ byName: z.string().min(1) }),
    z.object({ byIndex: z.number().int().nonnegative() }),
]);
...
export async function createCsvImportProfile(
    prisma: PrismaClient,
    input: CreateCsvImportProfileInput,
): Promise<CsvImportProfileView> {
...
export async function findMatchingCsvImportProfile(
    ...
    const row = await prisma.csvImportProfile.findFirst({
        where,
