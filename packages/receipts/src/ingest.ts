...
async function splitPercentsFor(prisma: PrismaClient, itemId: number, activeParticipants: Participant[]): Promise<number[]> {
...
async function resolveItem(
  ...
  const item =
    (extractedItem.itemCode
      ...
      : null) ?? (normalizedName ? await prisma.item.findUnique({ where: { storeId_normalizedName: { storeId, normalizedName } } }) : null);
  ...
  try {
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2002' && extractedItem.itemCode) {
      ...
      return { item: existing, isNew: false, splitPercents: await splitPercentsFor(prisma, existing.id, activeParticipants) };
    }
    throw error;
  }
}
