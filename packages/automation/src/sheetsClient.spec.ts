...
describe('SheetsClient.addTransactionsForPeriod', () => {
    it('writes rows into an existing sheet and finalizes them', async () => {
        ...
        const client = new SheetsClient({
            ...
            scriptId: SCRIPT_ID,
            sheets: { spreadsheets: fakeSheets },
            script: fakeScript,
        });
        ...
        expect(fakeSheets.batchUpdate).not.toHaveBeenCalledWith(
            /* eslint-disable @typescript-eslint/no-unsafe-assignment -- expect.objectContaining/anything are `any`-typed by design */
            expect.objectContaining({
                requestBody: expect.objectContaining({
                    requests: [expect.objectContaining({ duplicateSheet: expect.anything() })],
                }),
            }),
    ...
    it('creates a new sheet by duplicating "DUPLICATE ME" when none exists', async () => {
        ...
        expect(fakeSheets.batchUpdate).toHaveBeenCalledWith(
            expect.objectContaining({
                requestBody: {
                    requests: [
                        { duplicateSheet: { sourceSheetId: 100, insertSheetIndex: 1, newSheetName: 'Brian 07/26' } },
                    ],
                },
            }),
        );
    ...
    it('throws when no existing sheet and no "DUPLICATE ME" template', async () => {
        ...
        await expect(
            client.addTransactionsForPeriod({
                payerName: 'Brian',
                periodLabel: '07/26',
                rows: [['x', 'Brian', '1', 'Equally']],
            }),
    ...
    describe('rollback on a finalize failure', () => {
                ...
                expect.objectContaining({
                    requestBody: {
                        requests: [
                            {
                                deleteDimension: {
                                    range: { sheetId: 5, dimension: 'ROWS', startIndex: 3, endIndex: 4 },
                                },
                            },
                        ],
                    },
        ...
        it('reports both failures, without losing the original error, when rollback also fails', async () => {
            ...
            fakeSheets.batchUpdate
                .mockResolvedValueOnce({ data: {} })
                .mockRejectedValue(new Error('rollback network error'));
            const fakeScript = makeFakeScript({ message: 'boom' });
            const client = new SheetsClient({
                spreadsheetId: SPREADSHEET_ID,
                scriptId: SCRIPT_ID,
            ...
            await expect(
                ...
                /finalizeAddedRows failed \(finalizeAddedRows failed: boom\).*rollback also failed \(rollback network error\)/s,
            );
        });
    });
});
