...
export async function runSyncOverview(deps: RunSyncDeps): Promise<SyncRunResult> {
    ...
    try {
        ...
        await deps.prisma.csvSyncRun.create({
            data: { status: CsvSyncRunStatus.ERROR, periodResultsJson: '[]', errorMessage },
        });
    ...
    for (const [key, txs] of groups) {
        ...
        const rowPercentages = rows.map((row, i) =>
            txs[i].splitType === 'Variably' ? matchManifestEntry(row, payerName, manifestEntries) : null,
        );
        ...
        try {
            ...
            console.error(
                `[sync] ${payerName} ${periodLabel}: failed —`,
    ...
    const run = await deps.prisma.csvSyncRun.create({
        data: { status, periodResultsJson: JSON.stringify(periodResults) },
    });
