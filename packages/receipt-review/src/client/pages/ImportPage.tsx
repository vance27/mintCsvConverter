...
export function ImportPage() {
    ...
    async function pollJob(jobId: string, index: number): Promise<void> {
        for (;;) {
            ...
            if (job.status === 'done') {
                setFiles((prev) =>
                    prev.map((f, i) => (i === index ? { ...f, status: 'done', result: job.result } : f)),
                );
                return;
            }
            if (job.status === 'error') {
                setFiles((prev) =>
                    prev.map((f, i) => (i === index ? { ...f, status: 'error', message: job.message } : f)),
                );
                return;
            }
    ...
    async function handleSubmit(selected: File[]): Promise<void> {
        ...
        for (let index = 0; index < selected.length; index++) {
            ...
            if (profileId === null) {
                setFiles((prev) =>
                    prev.map((f, i) =>
                        i === index ? { ...f, status: 'error', message: 'Skipped — not configured' } : f,
                    ),
                );
                continue;
            }
            ...
            <Paper sx={{ p: 4 }}>
                <Stack spacing={3}>
                    ...
                    <Typography color="text.secondary">
                        Uploading a CSV export only stages transactions for review — it never syncs to Google Sheets by
                        itself.
                    </Typography>
                    <FormControl disabled={submitting} fullWidth>
                        <InputLabel id="import-payer-label">Payer</InputLabel>
                        <Select
                            labelId="import-payer-label"
                            label="Payer"
                            value={payer}
