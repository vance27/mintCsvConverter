          ...
          columnMapping: {
            ...
            amount:
              amountMode === 'DEBIT_CREDIT'
                ? { mode: 'DEBIT_CREDIT', debitColumn: { byIndex: debitColumn }, creditColumn: { byIndex: creditColumn } }
    ...
    <Paper sx={{ p: 4 }}>
      <Stack spacing={3}>
        ...
        <Typography color="text.secondary">
          "{fileName}" doesn't match any saved import profile. Map its columns below — this only needs to happen once per CSV shape.
        </Typography>
        ...
        <FormControlLabel control={<Checkbox checked={hasHeader} onChange={(e) => setHasHeader(e.target.checked)} />} label="First row is a header" />
        ...
        <Stack direction="row" spacing={2}>
          <FormControl fullWidth>
            ...
            <Select labelId="date-column-label" label="Date column" value={dateColumn} onChange={(e) => setDateColumn(Number(e.target.value))}>
              {columnIndexes.map((i) => (
        ...
        <RadioGroup row value={amountMode} onChange={(e) => setAmountMode(e.target.value as 'DEBIT_CREDIT' | 'SIGNED_AMOUNT')}>
