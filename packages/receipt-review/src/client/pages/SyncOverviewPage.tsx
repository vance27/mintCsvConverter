...
export function SyncOverviewPage() {
    ...
    async function refresh(): Promise<void> {
        const [overviewRes, runsRes, authRes] = await Promise.all([
            api['sync-overview'].$get(),
            api['sync-runs'].$get(),
            api['google-auth'].status.$get(),
        ]);
        ...
        <Container maxWidth="lg" sx={{ py: 6 }}>
            <Stack spacing={3}>
                ...
                <Stack direction="row" spacing={2} sx={{ alignItems: 'center' }}>
                    ...
                    {authStatus?.job?.status === 'error' ? (
                        <Alert severity="error">{authStatus.job.message}</Alert>
                    ) : null}
                </Stack>
                ...
                <Paper sx={{ p: 3 }}>
                    {overview === null ? (
                        ...
                        <Typography color="text.secondary">
                            Nothing to sync — every staged transaction is already synced.
                        </Typography>
                    ) : (
                        <Stack spacing={2}>
                            <Typography>
                                <strong>{overview.totalRows}</strong> row(s) across{' '}
                                <strong>{overview.groups.length}</strong> tab(s) will sync:
                            </Typography>
                            <TableContainer>
                                <Table size="small">
                                    <TableHead>
                                                ...
                                                <Stack spacing={0.5}>
                                                    {run.periodResults.map((p, i) => (
                                                        <Typography
                                                            key={i}
                                                            color={
                                                                p.status === 'FAILED' ? 'error.main' : 'text.primary'
                                                            }
                                                        >
